...
AS
BEGIN
 DECLARE @HavePermission NVARCHAR(500)  = '1' --(SELECT [dbo].[Ufn_UserCanHaveAccess](@OperationsPerformedBy,(SELECT OBJECT_NAME(@@PROCID))))
	...
	BEGIN
		...
		BEGIN
			...
			SET @CompanyId = (SELECT TOP(1)CompanyId FROM UserCompany WHERE UserId = @OperationsPerformedBy AND InActiveDateTime IS NULL )

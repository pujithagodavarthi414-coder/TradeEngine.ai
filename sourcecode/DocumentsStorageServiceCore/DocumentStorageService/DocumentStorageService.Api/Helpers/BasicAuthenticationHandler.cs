        ...
        public async Task<ValidUserOutputmodel> GetUserAuthTokenReadItem(string auth, string root)
        {
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    client.BaseAddress = new Uri(_configuration["AuthenticationServerApiUrl"] + "/api/LoginApi/GetRootPathAccess");
                    ...
                    if (response.IsSuccessStatusCode)
                    {
                        ...
                        return JsonConvert.DeserializeObject<ValidUserOutputmodel>(result);
                    }
                    else
                    {
                        return null;

CREATE FUNCTION [dbo].[Ufn_GetEmployeeTaxableAmountBasedOnSalaryAndTemplate]
(
...
BEGIN
	...
	DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId]((SELECT U.Id FROM [User] U INNER JOIN Employee E ON E.UserId = U.Id AND E.Id = @EmployeeId))), 
			...
			@StandardDeduction FLOAT = (SELECT StandardDeduction FROM TaxStandardDeduction),
	...
	WHERE TS.ParentId IS NULL
		...
		AND TS.TaxCalculationTypeId = (SELECT TaxCalculationTypeId FROM EmployeepayrollConfiguration WHERE SalaryId = @EmployeeSalaryId)
	...
	RETURN ROUND(ISNULL(@TaxableAmount,0),0)
	----Tax Calculation End

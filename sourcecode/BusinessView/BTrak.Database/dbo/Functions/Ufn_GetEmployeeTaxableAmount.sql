CREATE FUNCTION [dbo].[Ufn_GetEmployeeTaxableAmount]
(
...
BEGIN
	...
	FROM (
	SELECT *,
		   CASE WHEN IsCtcDependent = 1 THEN (CASE WHEN IsDeduction = 1 THEN -1 * (CASE WHEN IsPercentage = 1 THEN NetSalary * PercentageValue WHEN IsBands = 1 THEN dbo.Ufn_GetPayrollBandsAmountOfAComponent(PayrollComponentId,NetSalary,@RunStartDate,@RunEndDate,@EmployeeId) ELSE Amount END) ELSE 1 * (CASE WHEN IsPercentage = 1 THEN NetSalary * PercentageValue WHEN IsBands = 1 THEN dbo.Ufn_GetPayrollBandsAmountOfAComponent(PayrollComponentId,NetSalary,@RunStartDate,@RunEndDate,@EmployeeId) ELSE Amount END) END) 
	...
	BEGIN
		...
		FROM EmployeeTaxAllowances ETA 
		     INNER JOIN TaxAllowances TA ON TA.Id = ETA.TaxAllowanceId AND TA.InActiveDateTime IS NULL AND ETA.InActiveDateTime IS NULL
			            AND ( (TA.ToDate IS NOT NULL AND @YearStartDate BETWEEN TA.FromDate AND TA.ToDate AND @YearEndDate BETWEEN TA.FromDate AND TA.ToDate)
								OR (TA.ToDate IS NULL AND @YearEndDate >= TA.FromDate)
								OR (TA.ToDate IS NOT NULL AND @YearStartDate <= TA.ToDate AND @YearStartDate >= DATEADD(MONTH, DATEDIFF(MONTH, 0, TA.FromDate), 0))
							)
		...
		FROM TaxAllowances TA
		     INNER JOIN TaxAllowanceType TAT ON TAT.Id = TA.TaxAllowanceTypeId AND TAT.CompanyId = @CompanyId
		WHERE TA.Id IN (SELECT TA.ParentId FROM TaxAllowances TA INNER JOIN TaxAllowanceType TAT ON TAT.Id = TA.TaxAllowanceTypeId AND TAT.CompanyId = @CompanyId
		                 WHERE TAT.TaxAllowanceTypeName = 'Automatic'
						       AND ( (TA.ToDate IS NOT NULL AND @YearStartDate BETWEEN TA.FromDate AND TA.ToDate AND @YearEndDate BETWEEN TA.FromDate AND TA.ToDate)
								OR (TA.ToDate IS NULL AND @YearEndDate >= TA.FromDate)
								OR (TA.ToDate IS NOT NULL AND @YearStartDate <= TA.ToDate AND @YearStartDate >= DATEADD(MONTH, DATEDIFF(MONTH, 0, TA.FromDate), 0))
	...
	IF(@RunStartDate IS NOT NULL) SELECT @RunStartDate = DATEADD(DAY,-1,@RunStartDate)
	ELSE SELECT @RunStartDate = @YearEndDate
	...
	FROM PayrollRunEmployeeComponent PEC 
		 ...
		 INNER JOIN (SELECT PRE.EmployeeId, PR.PayrollStartDate, PR.PayrollEndDate, MAX(PRE.CreatedDateTime) CreatedDateTime
		           ...
		           AND DATEADD(month, DATEDIFF(month, 0, PR.PayrollEndDate), 0) BETWEEN @YearStartDate AND @RunStartDate
	WHERE PEC.ComponentName = 'Bonus' AND PEC.EmployeeId = @EmployeeId

	SELECT @EmployeeBonus = ISNULL(@EmployeeBonus,0) + IIF(@RunStartDate IS NOT NULL,ISNULL(@Bonus,0),0)
	...
	SELECT @EmployeeLeaveEncashment = ISNULL(@EmployeeLeaveEncashment,0) + IIF(@RunStartDate IS NOT NULL,ISNULL(@LeaveEncashment,0),0)
	...
	BEGIN
		...
		DECLARE @PaidTaxAmount FLOAT

		SELECT @PaidTaxAmount = PEC.ActualComponentAmount
		FROM PayrollRunEmployeeComponent PEC 
		     INNER JOIN PayrollRun PR ON PR.Id = PEC.PayrollRunId
		...
		WHERE PEC.ComponentName IN ('Tax')

		SELECT @TaxableAmount = ISNULL(@TaxableAmount,0) - ISNULL(@PaidTaxAmount,0)

	END
	
	RETURN CASE WHEN @TaxableAmount < 0 THEN 0 ELSE ROUND(ISNULL(@TaxableAmount,0),0) END
	----Tax Calculation End

END
GO

--SELECT dbo.[Ufn_GetEmployeeTaxableAmount]('A65D793B-D763-44E7-90B7-7436FA8B10B0',2020,'2020-03-01')
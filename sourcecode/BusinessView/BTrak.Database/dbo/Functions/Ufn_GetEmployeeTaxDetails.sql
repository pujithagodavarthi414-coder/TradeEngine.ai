--SELECT * from dbo.[Ufn_GetEmployeeTaxDetails]('993657E2-4B9C-4737-A560-84E547093FEE',2020,'2020-01-01','2020-12-01')
CREATE FUNCTION [dbo].[Ufn_GetEmployeeTaxDetails]
(
...
RETURNS @TaxDetails TABLE
(
	EmployeeId UNIQUEIDENTIFIER,
	EmployeeName NVARCHAR(MAX),
	NetSalary FLOAT,
	ParentSectionId UNIQUEIDENTIFIER,
	ParentSectionName NVARCHAR(MAX),
...
BEGIN
	...
	DECLARE @IsHavingInvestments BIT = 1

	DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId]((SELECT U.Id FROM [User] U INNER JOIN Employee E ON E.UserId = U.Id AND E.Id = @EmployeeId))), 
	        @EmployeeBranchId UNIQUEIDENTIFIER, @TaxableAmount FLOAT, @NetSalary FLOAT, @TotalInvestments FLOAT, @EmployeeJoinedDate DATE, @OneDaySalary FLOAT, @SalariesCount INT,
			@StandardDeduction FLOAT = (SELECT StandardDeduction FROM TaxStandardDeduction), @LatestPayrollTemplateId UNIQUEIDENTIFIER,
	...
	SELECT @LatestPayrollTemplateId = PayrollTemplateId
	FROM EmployeepayrollConfiguration
	WHERE SalaryId = (SELECT ES.Id
					  ...
					  WHERE ES.EmployeeId = @EmployeeId
					        ...
					        AND ( (ES.ActiveTo IS NOT NULL AND @YearStartDate BETWEEN ES.ActiveFrom AND ES.ActiveTo AND @YearEndDate BETWEEN ES.ActiveFrom AND ES.ActiveTo)
					  			  ...
					  			  OR (ES.ActiveTo IS NOT NULL AND @YearStartDate <= ES.ActiveTo AND @YearStartDate >= DATEADD(MONTH, DATEDIFF(MONTH, 0, ES.ActiveFrom), 0))
					  		    ) 
					  	    AND ES.CreatedDateTime = (SELECT MAX(ES.CreatedDateTime)		  
					  						     FROM EmployeeSalary ES
					  						     WHERE ES.EmployeeId = @EmployeeId
					  						           AND ES.InActiveDateTime IS NULL
					  						           AND ( (ES.ActiveTo IS NOT NULL AND @YearStartDate BETWEEN ES.ActiveFrom AND ES.ActiveTo AND @YearEndDate BETWEEN ES.ActiveFrom AND ES.ActiveTo)
	...
	DECLARE @LatestTaxCalculationTypeId UNIQUEIDENTIFIER = (SELECT TaxCalculationTypeId
	FROM EmployeepayrollConfiguration
	WHERE SalaryId = (SELECT ES.Id
					  FROM EmployeeSalary ES
					  WHERE ES.EmployeeId = @EmployeeId
	...
	SELECT @EmployeeJoinedDate = MAX(JoinedDate) FROM Job WHERE EmployeeId = @EmployeeId AND InActiveDateTime IS NULL
	...
	DECLARE @Gender NVARCHAR(100), @DateOfBirth DATE, @Age INT, @IsMale BIT
	...
	WHERE TS.ParentId IS NULL
	      AND TS.IsArchived = 0
		  AND ( (TS.ActiveTo IS NOT NULL AND @YearStartDate BETWEEN TS.ActiveFrom AND TS.ActiveTo AND @YearEndDate BETWEEN TS.ActiveFrom AND TS.ActiveTo)
		            OR (TS.ActiveTo IS NULL AND @YearEndDate >= TS.ActiveFrom)
					OR (TS.ActiveTo IS NOT NULL AND @YearStartDate <= TS.ActiveTo)
			     )
		...
		AND TS.TaxCalculationTypeId = @LatestTaxCalculationTypeId
	ORDER BY TS.[Order],TS.ActiveFrom

	DECLARE @ParentTaxSlabId UNIQUEIDENTIFIER, @IsFlatRate BIT

	SELECT TOP 1 @ParentTaxSlabId = ParentTaxSlabId, @IsFlatRate = IsFlatRate FROM @TaxSlabs WHERE [Order] = 1 ORDER BY ActiveFrom DESC
	...
	BEGIN
		...
		SELECT @TotalInvestments = 0

		SELECT @IsHavingInvestments = 0

	END
	----Investments Calculation End
	 
	SELECT @NetSalary = @NetSalary - ISNULL(@TotalInvestments,0) - @StandardDeduction
	...
	BEGIN
		...
		DECLARE @EductaionCessPercenatge FLOAT = (SELECT EducationCessPercentage FROM TaxEducationCess)  -- Eductaion Cess @ 4%

		SELECT @TaxableAmount = @TaxableAmount + (@TaxableAmount * @EductaionCessPercenatge * 0.01)

	END
	
	INSERT INTO @TaxDetails(EmployeeId,SectionId,SectionName,MaxInvestment,InvestedAmount,Investment)
	SELECT @EmployeeId,Id,Name,MaxAmount,InvestedAmount,Investment
	FROM @EmployeeInvestment

	INSERT INTO @TaxDetails(EmployeeId,ParentSectionId,SectionId,SectionName,MaxInvestment,InvestedAmount,Investment)
	SELECT @EmployeeId,ParentId,Id,Name,MaxAmount,InvestedAmount,NULL
	FROM @TaxAllowance
	WHERE ParentId IS NOT NULL

	UPDATE @TaxDetails SET ParentSectionName = TA.[Name]
	FROM @TaxDetails TD INNER JOIN  TaxAllowances TA ON TA.Id = TD.ParentSectionId

	UPDATE @TaxDetails SET TotalInvestments = @TotalInvestments
	                       , Incentives = ISNULL(@EmployeeBonus,0) + ISNULL(@EmployeeLeaveEncashment,0)
						   , TaxableAmount = @NetSalary, 
	                       Tax = ROUND(ISNULL(@TaxableAmount,0),0)
						   , NetSalary = (SELECT TOP 1 NetSalary FROM @EmployeeComponent)

	UPDATE @TaxDetails SET EmployeeName = U.FirstName + ' ' + ISNULL(U.SurName,'')
	FROM [User] U INNER JOIN Employee E On E.UserId = U.Id AND E.Id = @EmployeeId

	IF(@IsHavingInvestments = 0)
	BEGIN
	  
	  INSERT INTO @TaxDetails(EmployeeId,TotalInvestments,Incentives,TaxableAmount,Tax,NetSalary)
	  SELECT @EmployeeId,@TotalInvestments,ISNULL(@EmployeeBonus,0) + ISNULL(@EmployeeLeaveEncashment,0),@NetSalary,ROUND(ISNULL(@TaxableAmount,0),0),(SELECT TOP 1 NetSalary FROM @EmployeeComponent)

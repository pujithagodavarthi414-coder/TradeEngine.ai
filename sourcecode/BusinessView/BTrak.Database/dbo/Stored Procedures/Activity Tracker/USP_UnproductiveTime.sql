CREATE PROCEDURE [dbo].[USP_UnproductiveTime]
(
...
BEGIN TRY
  ...
  SELECT IIF(T.TotalTIme < 60,CAST(T.TotalTime AS NVARCHAR(50)) + 'm', CAST(CAST(ISNULL(T.TotalTime,0)/60.0 AS int) AS varchar(100))+'h'+IIF(CAST(ISNULL(T.TotalTime,0)%60 AS INT) = 0 ,'',CAST(CAST(ISNULL(T.TotalTime,0)%60 AS INT) AS VARCHAR(100))+'m')) [UnproductiveTime] 
	from [User] UDD 
	LEFT JOIN 
	(SELECT UM.Id AS UserId,UAT.CreatedDateTime AS CreatedDateTime
                        ,FLOOR(UnProductive * 1.0 / 60000 * 1.0) AS TotalTime
						FROM [User] AS UM
                           INNER JOIN (
                  SELECT UserId,CAST(CreatedDateTime AS date) CreatedDateTime ,SUM(UnProductive)UnProductive

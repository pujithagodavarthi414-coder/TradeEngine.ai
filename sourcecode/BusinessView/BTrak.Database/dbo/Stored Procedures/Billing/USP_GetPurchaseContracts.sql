CREATE PROCEDURE [dbo].[USP_GetPurchaseContracts]
(
        ...
        BEGIN
           ...
           DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId](@OperationsPerformedBy))       
		    ...
		    SELECT   C.Id AS ContractId,
					 C.ClientId,
					 C.[UniqueName] AS ContractUniqueName,
					 C.[ProductId],
					 C.[GradeId],
					 C.[Price] AS RateOrTon,
					 C.[TotalQuantity] AS ContractQuantity,
					 C.[UsedQuantity],
					 C.[RemaningQuantity],
					 C.[TimeStamp],
					 C.[CreatedDateTime],
					 ...
					 C.[ContractDocument],
					 C.[Description],
					 C.[StartDate] AS ContractDateFrom,
					 C.EndDate AS ContractDateTo,
		   ...
		   AND ((@SearchText IS NULL OR (U.SurName LIKE  @SearchText)) OR
				...
				(@SearchText IS NULL OR (C.TotalQuantity LIKE @SearchText)) OR
				(@SearchText IS NULL OR (C.StartDate) LIKE @SearchText) OR
				(@SearchText IS NULL OR (C.EndDate) LIKE @SearchText) OR
				(@SearchText IS NULL OR (CONCAT(U.FirstName ,' ',U.SurName) LIKE @SearchText)) OR
		   ...
		   ORDER BY
                          CASE WHEN( @SortDirection= 'ASC' OR @SortDirection IS NULL ) THEN
                               CASE 
                                    ...
                                    WHEN @SortBy = 'rateOrTon' THEN  C.[Price]
                                    WHEN @SortBy = 'contractQuantity' THEN C.TotalQuantity
									WHEN @SortBy = 'usedQuantity' THEN C.usedQuantity
									WHEN @SortBy = 'remaningQuantity' THEN  C.remaningQuantity
                                    WHEN @SortBy = 'contractDateFrom' THEN  CAST(C.StartDate AS SQL_VARIANT)
                                    WHEN @SortBy = 'contractDateTo' THEN  CAST(C.EndDate AS SQL_VARIANT)

...
-------------------------------------------------------------------------------
--EXEC [USP_GetUserStoryStatusReport] @GoalId = '491d88a3-6628-4d44-b377-520bb10a893a',@OperationsPerformedBy = 'EABC9A2D-4709-410B-8ABC-60C8EFFFE2F8'
-------------------------------------------------------------------------------
CREATE PROCEDURE [USP_GetUserStoryStatusReport]
(
...
BEGIN
    SET NOCOUNT ON
		...
		IF(@TimeZone IS NULL OR @TimeZone = '') SET @TimeZone =  'Asia/Kolkata'
     ...
     DECLARE @Currentdate DATETIMEOFFSET =   dbo.Ufn_GetCurrentTime(@Offset)    
		...
		SET @Offset = (SELECT STUFF(@Offset,4,0,':'))

        IF (@DateFrom IS NULL) SET @DateFrom  = (SELECT CONVERT(DATETIME,SWITCHOFFSET(MIN(US.CreatedDateTime),@Offset)) FROM UserStory US WHERE US.GoalId = @GoalId GROUP BY GoalId)
        IF (@DateTo IS NULL) SET  @DateTo  = (CASE WHEN EXISTS(SELECT  COUNT(1)
                                                               ...
                                                               JOIN UserStoryStatus USS ON USS.Id=US.UserStoryStatusId
                                                                ...
                                                                ) THEN SWITCHOFFSET(SYSDATETIMEOFFSET(),@Offset)
                                                    ELSE (SELECT SWITCHOFFSET(Max(US.CreatedDateTime),@Offset)
                                                          FROM UserStory US
                                                          JOIN UserStoryStatus USS ON USS.Id=US.UserStoryStatusId
                                                           AND US.GoalId= @GoalId
            ...
            IF (@DateTo > CAST(SWITCHOFFSET(SYSDATETIMEOFFSET(),@Offset) AS DATE)) SET @DateTo =CAST(SWITCHOFFSET(SYSDATETIMEOFFSET(),@Offset) AS DATE)
            DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId](@OperationsPerformedBy))
            DECLARE @Len INT = (SELECT MAX(LEN(UST.ShortName)) FROM UserStoryType UST JOIN UserStory US ON US.UserStoryTypeId = UST.Id AND US.GoalId = @GoalId AND UST.CompanyId = @CompanyId)
            ...
            DECLARE @WorkFlowId UNIQUEIDENTIFIER = (SELECT WorkFlowId FROM GoalWorkFlow WHERE GoalId = @GoalId GROUP BY WorkflowId)
           ...
           SELECT T.Date,Id,Z.UserStoryName,NULL,StatusColor,DeveloperName,Z.UserStoryUniqueName FROM  @Days T CROSS JOIN
			...
			FROM UserStory US INNER JOIN UserStoryStatus USS ON US.UserStoryStatusId = USS.Id AND  (@UserId IS NULL OR OwnerUserId = @UserId)
			                                                           LEFT JOIN [User]U ON U.Id = US.OwnerUserId  WHERE US.GoalId = @GoalId
        ...
        FOR SELECT Id FROM UserStory US WHERE US.GoalId = @GoalId
        ...
        DEALLOCATE Cursor_Script
           ...
           UPDATE @Temp SET SummaryValue = WFS.[OrderId] FROM WorkflowStatus WFS
                                                         ...
                                                         JOIN GoalWorkFlow GWF ON GWF.GoalId = @GoalId 
														 AND GWF.WorkflowId = WFS.WorkflowId AND GWF.InActiveDateTime IS NULL

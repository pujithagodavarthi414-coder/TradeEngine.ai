...
-------------------------------------------------------------------------------
--EXEC [USP_GetAuditQuestionsBasedOnVersionId] @OperationsPerformedBy='127133F1-4427-4149-9DD6-B02E0E036971'
-------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[USP_GetAuditQuestionsBasedOnVersionId]
(
	    ...
	    BEGIN
          ...
          SELECT AQ.Id AS QuestionId,
				...
				CVDC.Users SelectedEmployees,
                 ...
                 AR.RiskName,
                 AQ.RiskId,
                 NULL AS UpdatedByUserProfileImage,
                 QT.QuestionTypeName,
                 ...
                 (SELECT AA1.Id AS QuestionOptionId,
                        ...
                        ISNULL(AA1.QuestionOptionResult,0) AS QuestionOptionResult,
                         ...
                         FROM AuditAnswerVersions AA1 
                              JOIN QuestionTypeOptions QTP1 ON AA1.QuestionTypeOptionId = QTP1.Id AND AA1.AuditQuestionId = AQ.Id
                         AND QTP1.QuestionTypeId = AQ.QuestionTypeId 
                         AND QTP1.InActiveDateTime IS NULL
                         AND AA1.InActiveDateTime IS NULL
                         AND AA1.AuditComplianceVersionId = @AuditComplianceVersionId
          ...
          JOIN AuditCategoryVersions AC ON AC.Id = AQ.AuditCategoryId 
               AND AC.AuditComplianceVersionId = @AuditComplianceVersionId
               AND AQ.AuditComplianceVersionId = @AuditComplianceVersionId
               AND (AQ.Id = @QuestionId OR @QuestionId IS NULL) AND AC.InActiveDateTime IS NULL
          ...
          JOIN QuestionTypes QT ON QT.Id = AQ.QuestionTypeId AND QT.InActiveDateTime IS NULL
       	  JOIN AuditComplianceVersions A ON A.AuditId = AC.AuditComplianceId AND A.Id = @AuditComplianceVersionId
          ...
          LEFT JOIN #QuestionTypeFilter QTF ON QTF.QuestionTypeId = QT.MasterQuestionTypeId  
          ...
          WHERE (@AuditCategoryId IS NULL
                 OR (@IsHierarchical = 0 AND AC.Id = @AuditCategoryId) 
                 OR (@IsHierarchical = 1 AND AC.Id IN (SELECT AuditCategoryId FROM [dbo].[Ufn_GetChildAuditCategories](@AuditCategoryId,1,@AuditComplianceVersionId,1)))
                )

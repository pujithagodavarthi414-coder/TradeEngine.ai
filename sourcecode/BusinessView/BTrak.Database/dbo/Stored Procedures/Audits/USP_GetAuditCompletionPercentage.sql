CREATE PROCEDURE [dbo].[USP_GetAuditCompletionPercentage]
(
...
BEGIN
	...
	BEGIN
		...
		SELECT BranchName [Branch Name],
			   ...
			   ROUND(CAST((IIF(ISNULL(COUNT(1),0) = 0,0,ISNULL(SUM(IsCompleted)*1.0,0)/ISNULL(COUNT(1)*1.0,0)) * 100) AS NUMERIC(20,0)),0) [Completion Percentage]
		FROM (
				SELECT *,
					   CASE WHEN SubmittedDateTime IS NULL OR SubmittedDateTime > DeadlineDate THEN 0 ELSE 1 END IsCompleted
				FROM (
                    ...
                    FOR XML PATH(''), TYPE).value('.','NVARCHAR(MAX)'),1,1,''), 
										  B1.BranchName) BranchName, AC.Id AuditId, AC.AuditName,  ACT.AuditConductName, IIF(ACT.IsCompleted = 1,ACT.UpdatedDateTime,NULL) SubmittedDateTime, ACT.DeadlineDate
						...
						WHERE AC.CompanyId = @CompanyId
						      ...
						      AND (@BranchId IS NULL OR (ATT.TagId = @BranchId OR EB.BranchId = @BranchId))
				...
				GROUP BY AC.Id , AC.AuditName, ACT.Id , ACT.AuditConductName, ACT.IsCompleted ,ACT.UpdatedDateTime, ACT.DeadlineDate,B1.BranchName
				) T
		) TFinal
		GROUP BY BranchName, AuditName

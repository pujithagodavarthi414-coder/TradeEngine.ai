CREATE PROCEDURE [dbo].[USP_CloneAuditBasedOnVersionId]
(
    @AuditComplianceId UNIQUEIDENTIFIER = NULL --AuditId
    ,@AuditComplianceVersionId UNIQUEIDENTIFIER = NULL --AuditVersionId
    ,@AuditComplianceName NVARCHAR(250) = NULL --Auditname
    ,@IsArchived BIT = 0
...
BEGIN
      SET NOCOUNT ON
		 ...
		 DECLARE @ProjectId UNIQUEIDENTIFIER = (SELECT ProjectId FROM AuditComplianceVersions WHERE Id = @AuditComplianceVersionId)
		...
		BEGIN
			 ...
			 SET @MaxOrderQuestionId = (SELECT ISNULL(MAX(CAST(SUBSTRING(QuestionIdentity,3,LEN(QuestionIdentity)) AS INT)),0) + 1 
			                            FROM AuditQuestions WHERE CompanyId = @CompanyId)
			 ...
			 INSERT INTO [dbo].[AuditCompliance]([Id], [CompanyId], [AuditName], [InActiveDateTime], [CreatedDateTime], [CreatedByUserId], [Description], [IsRAG], [InboundPercent], [OutboundPercent], [CanLogTime],[ResposibleUserId],AuditFolderId, ProjectId)
				SELECT @CloneAuditId, @CompanyId, @CloneAuditName, NULL, GETDATE(), @OperationsPerformedBy, [Description], IsRAG, InboundPercent, OutboundPercent, CanLogTime , ResposibleUserId, AuditFolderId, ProjectId
					FROM AuditComplianceVersions
					WHERE Id = @AuditComplianceVersionId
			 ...
			 INSERT INTO AuditTags(Id, AuditId, TagId, TagName, CreatedByUserId, CreatedDateTime)
				SELECT NEWID(), @CloneAuditId, TagId, TagName, @OperationsPerformedBy, GETDATE()
					FROM AuditTagVersions 
					WHERE AuditComplianceVersionId = @AuditComplianceVersionId
			 ...
			 FROM AuditCategoryVersions 
			 WHERE AuditComplianceVersionId = @AuditComplianceVersionId
			 AND InActiveDateTime IS NULL
         
			 UPDATE #Temp SET MainParentAuditCategoryId = T.Parent
			 FROM #Temp AC 
			 INNER JOIN ( SELECT AC.AuditCategoryId,AC.MainAuditCategoryId,AC.ParentAuditCategoryId,ACC.MainAuditCategoryId AS Parent
			              FROM #Temp AC 
						       INNER JOIN #Temp ACC ON AC.ParentAuditCategoryId = ACC.AuditCategoryId
			 ...
			 INSERT INTO [dbo].[AuditCategory]([Id], [ParentAuditCategoryId], [AuditCategoryName]
			  ...
			  SELECT AC.MainAuditCategoryId, AC.MainParentAuditCategoryId, ACMain.AuditCategoryName
			         , ACMain.CreatedDateTime, @OperationsPerformedBy
					  , @CloneAuditId, ACMain.AuditCategoryDescription 
			  FROM #Temp AC
			  INNER JOIN AuditCategoryVersions ACMain ON ACMain.Id = AC.AuditCategoryId 
			             AND AuditComplianceVersionId = @AuditComplianceVersionId 
         
			  SELECT NEWID() AS NewQuestionId, AQ.Id AS OldQuesionId, AC.AuditCategoryId AS OldAuditCategoryId
			         , AC.MainAuditCategoryId AS NewAuditCategoryId
			  INTO #AuditQuestionTable
			  FROM AuditQuestionVersions AQ 
				   INNER JOIN #Temp AC ON AC.AuditCategoryId = AQ.AuditCategoryId 
				              AND AQ.InActiveDateTime IS NULL
							  AND AQ.[AuditComplianceVersionId] = @AuditComplianceVersionId
			  ...
			  INSERT INTO AuditQuestions (Id, QuestionName, AuditCategoryId, QuestionDescription, QuestionTypeId, CreatedByUserId, CreatedDateTime,
				...
				INSERT INTO [dbo].[Workspace](
									 ...
									 SELECT M.NewQuestionId,
											...
											@OperationsPerformedBy,
											@CompanyId
									  FROM #AuditQuestionTable M 
									       INNER JOIN AuditQuestions AQ ON AQ.Id = M.OldQuesionId AND AQ.InActiveDateTime IS NULL
									...
									FROM WorkspaceDashboards WD 
									     INNER JOIN #AuditQuestionTable M ON M.OldQuesionId = Wd.WorkspaceId 
			 ...
			 FROM AuditQuestions AQ
			 INNER JOIN (SELECT Id, ROW_NUMBER() OVER(ORDER BY CreatedDateTime) AS QNumber 
			             FROM AuditQuestions AQ
			 ...
			 FROM AuditAnswerVersions ANS --ON ANS.AuditQuestionId = AQ.Id AND ANS.InactiveDateTime IS NULL
			 	INNER JOIN #AuditQuestionTable AQT ON AQT.OldQuesionId = ANS.AuditQuestionId 
			 	           AND ANS.InactiveDateTime IS NULL
			 			   AND ANS.AuditComplianceVersionId = @AuditComplianceVersionId

CREATE PROCEDURE [dbo].[USP_GetAuditsFolderView]
(
   ...
   DECLARE @HavePermission NVARCHAR(250)  = (SELECT [dbo].[Ufn_UserCanHaveEntityFeatureAccess](@OperationsPerformedBy,@ProjectId,(SELECT OBJECT_NAME(@@PROCID))))
		...
		IF(@BusinessUnitIds = '') SET @BusinessUnitIds = NULL
		  ...
		  (
				...
				AuditWorkFlowId UNIQUEIDENTIFIER
		  )
		  ...
		  LEFT JOIN (SELECT A.Id AS AuditId,ISNULL(T.Permission,1) Permission FROM AuditCompliance A 
			       LEFT JOIN(
							...
							--WHERE (@BusinessUnitIds IS NULL OR E.Id IN (SELECT DISTINCT EmployeeId FROM [Ufn_GetAccessibleMembersForBusinessUnit](@EmployeeId,@BusinessUnitIds,@CompanyId)))
								   ) T ON T.AuditId = A.Id
		 ...
		 ORDER BY AuditName ASC
				...
				,AC.ProjectId
		 INSERT INTO #AuditComplianceWithFolders(ProjectId,AuditId,AuditName,ParentAuditId,ParentAuditName,[FolderTimeStamp],IsAudit, CreatedDateTime, UpdatedDateTime)

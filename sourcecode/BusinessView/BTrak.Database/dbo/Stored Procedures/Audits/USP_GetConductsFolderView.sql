CREATE PROCEDURE [dbo].[USP_GetConductsFolderView]
(
	...
	BEGIN TRY
		...
		BEGIN
							   ...
							   LEFT JOIN(
										 SELECT AG.AuditId,IIF(AG.TagId = @OperationsPerformedBy OR UR.Id IS NOT NULL OR EB.Id IS NOT NULL OR BU.BusinessUnitId IS NOT NULL,1,0) AS Permission
												...
												WHERE (@BranchId IS NULL OR @BranchId = EB.BranchId)
												      --AND (@BusinessUnitIds IS NULL OR E.Id IN (SELECT DISTINCT EmployeeId FROM [Ufn_GetAccessibleMembersForBusinessUnit](@EmployeeId,@BusinessUnitIds,@CompanyId)))
			                 					) T ON T.AuditId = A.Id
		 ...
		 --BEGIN
			...
			--INSERT INTO #AuditConductWithFolders(ConductId,AuditConductName,ParentConductId,ParentAuditConductName,[FolderTimeStamp],Lvl,IsConduct, CreatedDateTime)
			--SELECT APF.Id,APF.AuditFolderName,APF.ParentAuditFolderId,AF.AuditFolderName,APF.[TimeStamp],lvl,0, APF.CreatedDateTime
			...
			--      AND APF.InActiveDateTime IS NULL
		 ...
		 INSERT INTO #AuditConductWithFolders(ProjectId,ConductId,AuditConductName,ParentConductId,ParentAuditConductName,[FolderTimeStamp],IsConduct, CreatedDateTime, UpdatedDateTime)
		 SELECT AF.ProjectId,AF.Id,AF.AuditFolderName,AF.ParentAuditFolderId,APF.AuditFolderName,AF.[TimeStamp],0, AF.CreatedDateTime, AF.UpdatedDateTime
		 FROM AuditFolder AF
		      LEFT JOIN AuditFolder APF ON APF.Id = AF.ParentAuditFolderId
		 WHERE AF.InActiveDateTime IS NULL 
		       AND AF.CompanyId = @CompanyId
			   AND (@ProjectId IS NULL OR AF.ProjectId = @ProjectId)

CREATE PROCEDURE [dbo].[USP_GetAuditImmediatePriorities]
(
	...
	BEGIN
		...
		FROM (
				SELECT ISNULL(STUFF((SELECT ',' + B.BranchName
                    ...
                    FOR XML PATH(''), TYPE).value('.','NVARCHAR(MAX)'),1,1,''),B1.BranchName) BranchName, ACT.Id AuditConductId, ACT.AuditConductName, IIF(ACT.IsCompleted = 1,ACT.UpdatedDateTime,NULL) SubmittedDateTime, ACT.DeadlineDate
		...
		GROUP BY  B.BranchName, ACT.Id , ACT.AuditConductName, ACT.IsCompleted ,ACT.UpdatedDateTime, ACT.DeadlineDate,B1.BranchName
		) T
		WHERE SubmittedDateTime IS NULL OR SubmittedDateTime > DeadlineDate
		GROUP BY BranchName

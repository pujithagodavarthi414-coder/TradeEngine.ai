...
-------------------------------------------------------------------------------
--EXEC [dbo].[USP_LeaveTransactionReport] @OperationsPerformedBy = 'CB900830-54EA-4C70-80E1-7F8A453F2BF5'
-------------------------------------------------------------------------------
CREATE PROCEDURE [dbo].[USP_LeaveTransactionReport]
(
			...
			IF(@IsActiveEmployeesOnly =1)
				...
				SET  @SqlQuery = @SqlQuery + ' INNER JOIN LeaveType LT ON LT.Id = LA.LeaveTypeId
										...
										WHERE U.CompanyId = @CompanyId
											  ...
											  AND CONVERT(DATE,LA.UpdatedDateTime) > LA.LeaveDateFrom'
		
		SET @SqlQuery = @SqlQuery + ') InnerSql'

		DECLARE @FinalSql NVARCHAR(MAX) = 'SELECT ' + @ColumnString + '  FROM ' + @SqlQuery + ' ORDER BY [LeaveDateFrom]'

...
CREATE PROCEDURE [dbo].[USP_GetDelayedWorkItems]
(
         ...
         SELECT US.Id UserStoryId,
          ...
          FROM UserStory US        INNER JOIN Project P ON P.Id = US.ProjectId AND P.InActiveDateTime IS NULL
		                         LEFT JOIN Goal G ON G.Id = US.GoalId AND US.InActiveDateTime IS NULL
         						 ...
         						 LEFT JOIN Sprints S ON S.Id = US.SprintId AND S.InActiveDateTime IS NULL AND ((S.IsReplan = 0 OR S.IsReplan IS NULL) AND S.SprintStartDate IS NOT NULL AND (S.IsComplete = 0 OR S.IsComplete IS NULL))
								 ...
								 INNER JOIN [User]U1 ON U1.Id = P.ProjectResponsiblePersonId AND U.InActiveDateTime IS NULL
         						 WHERE P.Id  IN (SELECT ReferenceId FROM AutomatedWorkFlow AW INNER JOIN WorkflowTrigger WT ON WT.WorkflowId = AW.Id   
         						         ...
         						         AND AW.CompanyId = (SELECT Id FROM Company WHERE CompanyName = @CompanyName and InactiveDateTime is null) 
         						           AND WT.InactiveDateTime IS NULL AND AW.InactiveDateTime IS NULL
         								           AND AW.WorkflowName ='Task Delay Notification Workflow'
												   AND T.TriggerName = 'TaskDelayNotificationTrigger'
												   AND WT.RefereceTypeId ='26A91CCF-504A-479C-8E8F-92128FB03AF6')
												   AND ((US.GoalId IS NOT NULL AND US.DeadLineDate  < GETDATE()) OR (US.SprintId IS NOT NULL AND S.SprintEndDate < GETDATE())) 
	END	
	END TRY
	BEGIN CATCH

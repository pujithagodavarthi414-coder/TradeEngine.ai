--EXEC [USP_GetHourlyEmployeeRates] '1CDB3294-9B0F-479A-A6CD-CD61A7240FAB','2020-08-01','2020-08-31'

CREATE PROCEDURE [dbo].[USP_GetHourlyEmployeeRates]
(
...
BEGIN
	...
	SELECT ROW_NUMBER() OVER (ORDER BY E.Id) Id,E.Id EmployeeId, E.UserId, DR.DateData [Date], DATENAME(WEEKDAY,DR.DateData) [DayOfWeek], SW.Id ShiftWeekId, ST.Id ShiftTimimgId, ST.ShiftName, SW.StartTime, SW.EndTime,
		   ...
		   CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(CONVERT( DATETIME2, SWITCHOFFSET(TS.InTime,'+00:00'), 1) AS TIME) AS DATETIME) InTime, 
		   CASE WHEN CAST(CONVERT( DATETIME2, SWITCHOFFSET(TS.InTime,'+00:00'), 1) AS TIME) > CAST(CONVERT( DATETIME2, SWITCHOFFSET(TS.OutTime,'+00:00'), 1) AS TIME) THEN CAST(CAST(DATEADD(DAY,1,TS.[Date]) AS DATE) AS DATETIME) + CAST(CAST(CONVERT( DATETIME2, SWITCHOFFSET(TS.OutTime,'+00:00'), 1) AS TIME) AS DATETIME) 
		        ELSE CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(CONVERT( DATETIME2, TS.OutTime, 1) AS TIME) AS DATETIME) END OutTime, 
		   CONVERT( DATETIME2, SWITCHOFFSET(TS.LunchBreakStartTime,'+00:00'), 1) LunchBreakStartTime,CONVERT( DATETIME2,SWITCHOFFSET(TS.LunchBreakEndTime,'+00:00'), 1) LunchBreakEndTime, 
		   ISNULL(UBInner.BreakMins,0) + ISNULL(DATEDIFF(MINUTE,SWITCHOFFSET(TS.LunchBreakStartTime,'+00:00'),SWITCHOFFSET(TS.LunchBreakEndTime,'+00:00')),0) BreakInMin, 
		   WD.IsWeekEnd, IIF(H.Id IS NULL,0,1) IsBankHoliday, 1 IsRegularDay
	INTO #EmployeeDetails
	FROM DateRange DR
	     INNER JOIN Employee E ON 1 = 1 AND E.InActiveDateTime IS NULL
		...
		INNER JOIN EmployeeBranch EB ON EB.EmployeeId = E.Id
		      AND ( (EB.ActiveTo IS NOT NULL AND DR.DateData BETWEEN EB.ActiveFrom AND EB.ActiveTo)
			          OR (EB.ActiveTo IS NULL AND DR.DateData >= EB.ActiveFrom)
					  OR (EB.ActiveTo IS NOT NULL AND DR.DateData <= EB.ActiveTo)
				  ) 
		INNER JOIN EmploymentStatus ESS ON ESS.Id = J.EmploymentStatusId AND ESS.InActiveDateTime IS NULL
		INNER JOIN WeekDays WD ON WD.WeekDayName = DATENAME(WEEKDAY,DR.DateData) AND WD.CompanyId = @CompanyId
		INNER JOIN Branch B ON B.Id = EB.BranchId
		--INNER JOIN Region R ON R.Id = B.RegionId
		LEFT JOIN TimeSheet TS ON TS.UserId = U.Id AND TS.[Date] = DR.DateData
		LEFT JOIN (SELECT UserId,[Date],SUM(DATEDIFF(MINUTE,SWITCHOFFSET(BreakIn,'+00:00'),SWITCHOFFSET(BreakOut,'+00:00'))) BreakMins FROM UserBreak
                   GROUP BY UserId,[Date]) UBInner ON UBInner.UserId = TS.UserId AND UBInner.[Date] = TS.[Date]
		LEFT JOIN Holiday H ON H.[Date] = DR.DateData AND H.CompanyId = @CompanyId AND H.CountryId = B.CountryId
		...
		LEFT JOIN ShiftWeek SW ON SW.ShiftTimingId = ST.Id AND SW.[DayOfWeek] = DATENAME(WEEKDAY,DR.DateData)
	...
	UPDATE #EmployeeDetails SET InTime = CAST(CAST(ED.[Date] AS DATE) AS DATETIME) + CAST(CAST(TS.StartTime AS TIME) AS DATETIME), 
		                        OutTime = CASE WHEN CAST(TS.StartTime AS TIME) > CAST(TS.EndTime AS TIME) THEN CAST(CAST(DATEADD(DAY,1,ED.[Date]) AS DATE) AS DATETIME) + CAST(CAST(TS.EndTime AS TIME) AS DATETIME) 
		                                       ELSE CAST(CAST(ED.[Date] AS DATE) AS DATETIME) + CAST(CAST(TS.EndTime AS TIME) AS DATETIME) END,
								BreakInMin = BreakMins, LunchBreakStartTime = NULL, LunchBreakEndTime = NULL
	FROM #EmployeeDetails ED 
	     INNER JOIN TimeSheetPunchCard TS ON TS.UserId = ED.UserId AND TS.[Date] = ED.[Date]
	...
	SELECT ROW_NUMBER() OVER(ORDER BY ERF.EmployeeId) Id, ERF.EmployeeId, ERF.[Date], ERFInner.InTime StartTime, ERFInner.OutTime EndTime, ERF.NetRate
	INTO #EmployeeSalaryWithAllowances
	FROM (
			SELECT EmployeeId, [Date],ROUND((SUM(FinalRate) + ISNULL(SUM(BreakAllowance),0) + ISNULL(SUM(AdditionalAllowance),0)),2) NetRate
			FROM #EmployeeFinalRates 
			GROUP BY EmployeeId,[Date]
	) ERF
      INNER JOIN (SELECT EmployeeId, [Date], InTime, OutTime FROM #EmployeeFinalRates WHERE InTime IS NOT NULL AND OutTime IS NOT NULL
	              GROUP BY EmployeeId, [Date], InTime, OutTime) ERFInner ON ERF.EmployeeId = ERFInner.EmployeeId AND ERF.[Date] = ERFInner.[Date]
	
	
	SELECT * FROM #EmployeeSalaryWithAllowances ORDER BY EmployeeId,[Date]

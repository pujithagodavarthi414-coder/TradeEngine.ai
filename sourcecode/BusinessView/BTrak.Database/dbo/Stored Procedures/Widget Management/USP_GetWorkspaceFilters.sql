...
CREATE PROCEDURE [dbo].[USP_GetWorkspaceFilters]
(
	...
	BEGIN TRY
        ...
        BEGIN
			...
			DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId](@OperationsPerformedBy))
			   ...
			   CREATE TABLE #AppWorkspaceFilters 
				(
				...
				INSERT INTO #AppWorkspaceFilters(FilterId,FilterKey,FilterValue,FilterName,IsSystemFilter)
				SELECT  WF.WorkspaceParameterId,WP.ParameterName,WF.FilterValue,WP.ParameterLabel,WP.IsSystemFilter
				FROM WorkspaceFilter WF
					INNER JOIN WorkspaceParameter WP ON WP.Id = WF.WorkspaceParameterId
                    AND WF.CreatedByUserId = @OperationsPerformedBy AND @DashboardAppId = WP.ReferenceId

				CREATE TABLE #WorkspaceFilters 
				(
					FilterId UNIQUEIDENTIFIER
					,FilterKey NVARCHAR(250)
					,FilterValue NVARCHAR(250)
				...
				INSERT INTO #WorkspaceFilters(FilterId,FilterKey,FilterValue,FilterName,IsSystemFilter)
				SELECT WF.WorkspaceParameterId,WP.ParameterName,WF.FilterValue,WP.ParameterLabel,WP.IsSystemFilter
				FROM WorkspaceFilter WF
					INNER JOIN WorkspaceParameter WP ON WP.Id = WF.WorkspaceParameterId
			  ...
			  SELECT FilterId,FilterKey,ISNULL(FilterValue,DashboardFilterValue) AS FilterValue,FilterName,IsSystemFilter
			  FROM #AppWorkspaceFilters
			  UNION
			  SELECT FilterId,FilterKey,FilterValue,FilterName,IsSystemFilter

CREATE PROCEDURE [dbo].[Marker112]
(
...
SET NOCOUNT ON
	...
	UPDATE WorkspaceDashboards SET [Name] = 'Regression test run report' WHERE  [Name] = 'Testrun details' 
AND CompanyId = @CompanyId AND CustomWidgetId IN (SELECT Id FROM  CustomWidgets WHERE  CustomWidgetName = 'Testrun details'  AND CompanyId = @CompanyId)
UPDATE CustomWidgets SET CustomWidgetName = 'Regression test run report' 
WHERE  CustomWidgetName ='Testrun details'  AND CompanyId = @CompanyId	

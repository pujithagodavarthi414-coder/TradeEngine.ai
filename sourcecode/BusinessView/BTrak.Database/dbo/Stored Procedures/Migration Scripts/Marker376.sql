CREATE PROCEDURE [dbo].[Marker376]
(
...
BEGIN
	...
	USING ( VALUES
	       (NEWID(), N'Vessel Owner', @CompanyId, GETDATE(), @UserId, NULL,4),
		   (NEWID(), N'Ship Broker', @CompanyId, GETDATE(), @UserId, NULL,5)
	        ) 
	...
	USING ( VALUES
	       (NEWID(), @RoleId, (SELECT TOP 1 Id FROM ClientType WHERE ClientTypeName='Vessel Owner' AND CompanyId=@CompanyId), @CompanyId, GETDATE(), @UserId, NULL),
		   (NEWID(), @RoleId, (SELECT TOP 1 Id FROM ClientType WHERE ClientTypeName='Ship Broker' AND CompanyId=@CompanyId), @CompanyId, GETDATE(), @UserId, NULL)
	      ) 

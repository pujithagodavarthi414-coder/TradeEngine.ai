CREATE PROCEDURE [dbo].[Marker220]
(
...
FROM AuditConduct AC  INNER JOIN AuditCompliance AA ON AC.AuditComplianceId = AA.Id  
WHERE   AA.CompanyId = ''@CompanyId''   
AND (''@ProjectId'' = '''' OR AA.ProjectId = ''@ProjectId'') 
AND (''@BusinessUnitIds'' = '''' OR AC.Id IN (SELECT AuditConductId FROM AuditTags WHERE TagId IN (SELECT BusinessUnitId FROM [dbo].[Ufn_GetAccessibleBusinessUnits](''@OperationsPerformedBy'',''@CompanyId'',''@BusinessUnitIds'')) 
...
UPDATE CustomWidgets SET WidgetQuery = 'SELECT COUNT(1) [Audits due] 
...
AND CAST(AC.CreatedDateTime AS date) >= CAST(ISNULL(@DateFrom,DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0))  AS date)     
AND CAST(AC.CreatedDateTime AS date) <= CAST(ISNULL(@DateTo,DATEADD(yy, DATEDIFF(yy, 0, GETDATE()) + 1, -1)) AS date)' 
	WHERE CustomWidgetName = 'Audits due count' AND CompanyId = @CompanyId

UPDATE CustomWidgets SET WidgetQuery = 'SELECT COUNT(1) [Inprogress audits]
FROM AuditConduct AC     
...
AND (''@BusinessUnitIds'' = '''' OR AC.Id IN (SELECT AuditConductId FROM AuditTags WHERE TagId IN (SELECT BusinessUnitId FROM [dbo].[Ufn_GetAccessibleBusinessUnits](''@OperationsPerformedBy'',''@CompanyId'',''@BusinessUnitIds''))  AND InActiveDateTime IS NULL))
AND (''@AuditId'' = '''' OR AA.Id = ''@AuditId'')' 
	WHERE CustomWidgetName = 'Inprogress audits' AND CompanyId = @CompanyId

UPDATE CustomWidgets SET WidgetQuery = 'SELECT COUNT(1) [Audits Overdue] 
FROM AuditConduct  AC 
INNER JOIN AuditCompliance ACT ON ACT.Id = AC.AuditComplianceId  
AND AC.InActiveDateTime IS NULL 
AND ACT.InActiveDateTime IS NULL  
...
AND (''@BusinessUnitIds'' = '''' OR AC.Id IN (SELECT AuditConductId FROM AuditTags WHERE TagId IN (SELECT BusinessUnitId FROM [dbo].[Ufn_GetAccessibleBusinessUnits](''@OperationsPerformedBy'',''@CompanyId'',''@BusinessUnitIds''))  AND InActiveDateTime IS NULL))
AND (''@AuditId'' = '''' OR ACT.Id = ''@AuditId'')' 
	WHERE CustomWidgetName = 'Audits Overdue' AND CompanyId = @CompanyId
...
UPDATE CustomWidgets SET WidgetQuery = 'SELECT StatusCount ,StatusCounts  
 from (SELECT COUNT(CASE WHEN AC.IsCompleted = 1 AND CAST(AC.CreatedDateTime AS date)   = CAST(AQH.CreatedDateTime AS date) 
	  ...
	  UNPIVOT (StatusCounts FOR StatusCount IN ([Single Attempt Completed Count],[Created Count],[Inprogress Count]))p' 
	WHERE CustomWidgetName = 'Audits overview' AND CompanyId = @CompanyId

UPDATE CustomWidgets SET WidgetQuery = 'SELECT COUNT(1) [Upcoming audits] 
FROM  AuditConduct AC 
INNER JOIN AuditCompliance AA ON AC.AuditComplianceId = AA.Id 
AND AC.InActiveDateTime IS NULL 
AND AA.InActiveDateTime IS NULL
...
AND CAST(DeadlineDate AS date) >=   CAST( GETDATE() AS date))' 
	WHERE CustomWidgetName = 'Upcoming audits' AND CompanyId = @CompanyId
...
LEFT JOIN   (select COUNT(CASE WHEN  QuestionOptionResult = 1 THEN 1 END) PassedCount 
			  ...
			  GROUP BY FORMAT(T.Date,''MMM-yy''),T.Date' 
	WHERE CustomWidgetName = 'Audit compliance percentage month wise' AND CompanyId = @CompanyId

UPDATE CustomWidgets SET WidgetQuery = 'SELECT FORMAT(T.Date,''MMM-yyyy'') Date
...
LEFT JOIN [AuditQuestionHistory] AQH ON AQH.ConductId = AC.Id  
		  ...
		  AND (''@BusinessUnitIds'' = '''' 
		        OR AC.Id IN (SELECT AuditConductId 
				             FROM AuditTags 
							 WHERE TagId IN (SELECT BusinessUnitId FROM [dbo].[Ufn_GetAccessibleBusinessUnits](''@OperationsPerformedBy'',''@CompanyId'',''@BusinessUnitIds''))  
							       AND InActiveDateTime IS NULL))
		  AND CAST(AC.CreatedDateTime AS date) >= CAST(ISNULL(@DateFrom,DATEADD(yy, DATEDIFF(yy, 0, GETDATE()), 0))  AS date)  
		  AND CAST(AC.CreatedDateTime AS date) <= CAST(ISNULL(@DateTo,DATEADD(yy, DATEDIFF(yy, 0, GETDATE()) + 1, -1)) AS date)  
		  GROUP BY FORMAT(AC.CreatedDateTime,''MMM-yyyy''))R ON FORMAT(T.Date,''MMM-yyyy'') = R.CreatedDateTime' 
	WHERE CustomWidgetName = 'Audits created and submitted on same day' AND CompanyId = @CompanyId

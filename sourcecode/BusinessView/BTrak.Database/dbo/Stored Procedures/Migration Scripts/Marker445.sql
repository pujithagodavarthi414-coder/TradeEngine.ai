CREATE PROCEDURE [dbo].[Marker445]
(
...
USING ( VALUES 
 (NEWID(),N'By using this app we can calculate the position,profit and loss.', N'Positions and P&L', CAST(N'2023-03-17 09:57:46.710' AS DateTime),@UserId,@CompanyId,NULL,NULL,NULL)
 )
...
MERGE INTO [dbo].[WidgetModuleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE WidgetName = 'Positions and P&L' AND CompanyId = @CompanyId),'3DC59A11-F589-42A5-8F7A-925EC9B9542B',@UserId,@Currentdate)
	...
	MERGE INTO [dbo].[WidgetRoleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE CompanyId = @CompanyId AND WidgetName = 'Positions and P&L'),@RoleId,GETDATE(),@UserId)

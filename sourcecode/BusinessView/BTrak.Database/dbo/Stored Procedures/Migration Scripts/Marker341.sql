CREATE PROCEDURE [dbo].[Marker341]
(
...
BEGIN
    ...
    USING ( VALUES 
     (NEWID(),'This app can be used to Bank account', N'Bank accounts', CAST(N'2021-10-07 19:27:24.523' AS DateTime),@UserId,@CompanyId,NULL,NULL,NULL)
    )
	...
	MERGE INTO [dbo].[WidgetModuleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE WidgetName = 'Bank accounts' AND CompanyId = @CompanyId),'EF94A114-B7A0-49B8-B4F7-D99F1E121C4F',@UserId,@Currentdate)
	 )
	...
	MERGE INTO [dbo].[WidgetRoleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE CompanyId = @CompanyId AND WidgetName = 'Bank accounts'),@RoleId,GETDATE(),@UserId)
	 )
    ...
    USING ( VALUES
		(NEWID(), @RoleId, N'035DFB11-C824-4A77-B912-06904FFD1261', CAST(N'2021-10-08 17:39:32.367' AS DateTime),@UserId)
    )
	...
	USING ( VALUES 
     (NEWID(),(SELECT TOP 1 Id FROM Workspace WHERE WorkspaceName  = 'Customized_advancedinvoices' AND CompanyId = @CompanyId),0,0, 27,16, 5,  5, 4,  'Bank accounts',NULL,0,NULL,@UserId,GETDATE() ,@CompanyId)

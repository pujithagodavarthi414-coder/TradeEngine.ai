CREATE PROCEDURE [dbo].[Marker285]
(
...
BEGIN
	...
	SELECT TOP(1000) * FROM (SELECT UA.UserId,ROUND(SUM(( DATEPART(HH, SpentTime) * 3600000 ) +  
	...
	GROUP BY UA.UserId,UA.AbsoluteAppName) T ORDER BY SpentInHours DESC'
	WHERE CompanyId = @CompanyId
	AND Customwidgetname = 'My app usage'
	...
	GROUP BY UA.UserId,UA.AbsoluteAppName) T ORDER BY SpentInHours DESC'
	WHERE CompanyId = @CompanyId
	AND Customwidgetname = 'My web usage'
	...
	UPDATE CustomWidgets SET WidgetQuery ='
	 SELECT Da.[Date],ISNULL(P.Productive,0)Productive,ISNULL(P.UnProductive,0)UnProductive,ISNULL(P.Neutral,0)Neutral 

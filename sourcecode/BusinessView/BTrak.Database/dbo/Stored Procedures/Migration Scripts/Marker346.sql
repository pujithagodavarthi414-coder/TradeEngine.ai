CREATE PROCEDURE [dbo].[Marker346]
(
...
BEGIN
	...
	MERGE INTO [dbo].[MessageFieldMaster] AS Target 
    USING ( VALUES 
		(NEWID(), 'Thank you', 'Merci pour votre règlement de notre dernière facture.', NULL, 1, GETDATE(),@UserId, @CompanyId),
		(NEWID(), 'General', '', NULL, 1, GETDATE(),@UserId, @CompanyId),
		(NEWID(), 'Reminder', 'Nous tenons également à vous rappeler notre précédente facture impayée d''une valeur de CHF', NULL, 1, GETDATE(),@UserId, @CompanyId)
	)
    AS Source ([Id], MessageType, DisplayText, GrdId, IsDisplay, [CreatedDateTime], [CreatedByUserId],[CompanyId]) 
    ON Target.MessageType = Source.MessageType  AND Target.[CompanyId] = Source.[CompanyId]
    WHEN MATCHED THEN 
    ...
    INSERT ([Id], MessageType, DisplayText, GrdId, IsDisplay, [CreatedDateTime], [CreatedByUserId],[CompanyId]) 
    VALUES ([Id], MessageType, DisplayText, GrdId, IsDisplay, [CreatedDateTime], [CreatedByUserId],[CompanyId]);

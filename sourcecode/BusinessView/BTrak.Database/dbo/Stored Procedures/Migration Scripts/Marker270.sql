CREATE PROCEDURE [dbo].[Marker270]
(
...
SET NOCOUNT ON
    ...
    USING ( VALUES       
        (NEWID(),N'This app displays the daily offline users based on permission. If permission exists it will display all users else reporting users only',N'Offline Users', GETDATE(),@UserId,@CompanyId,NULL,NULL,NULL)        
	)
    ...
    VALUES([Id],[Description], [WidgetName], [CreatedDateTime],[CreatedByUserId],[CompanyId],[UpdatedDateTime],[UpdatedByUserId],[InActiveDateTime]) ;
     ...
     USING (VALUES     
     (NEWID(),(SELECT Id FROM Widget WHERE WidgetName = N'Offline Users' AND CompanyId = @CompanyId),(SELECT Id FROM Module WHERE ModuleName = 'Activity tracker' ),@UserId,GETDATE())
     )

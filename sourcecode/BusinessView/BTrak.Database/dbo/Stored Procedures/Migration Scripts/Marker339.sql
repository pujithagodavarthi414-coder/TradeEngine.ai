CREATE PROCEDURE [dbo].[Marker339]
(
...
BEGIN
    ...
    USING ( VALUES 
     (NEWID(),'This app can be used to Entry Form', N'Entry Form', CAST(N'2021-10-07 19:27:24.523' AS DateTime),@UserId,@CompanyId,NULL,NULL,NULL)
    )
	...
	MERGE INTO [dbo].[WidgetModuleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE WidgetName = 'Entry Form' AND CompanyId = @CompanyId),'EF94A114-B7A0-49B8-B4F7-D99F1E121C4F',@UserId,@Currentdate)
	 )
	...
	MERGE INTO [dbo].[WidgetRoleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE CompanyId = @CompanyId AND WidgetName = 'Entry Form'),@RoleId,GETDATE(),@UserId)
	 )
    ...
    USING ( VALUES
		(NEWID(), @RoleId, N'C8ECFF90-A5FC-424C-89EC-28B2EAB9E0FF', CAST(N'2021-10-08 17:39:32.367' AS DateTime),@UserId)
    )
	...
	USING ( VALUES 
     (NEWID(),(SELECT TOP 1 Id FROM Workspace WHERE WorkspaceName  = 'Customized_advancedinvoices' AND CompanyId = @CompanyId),0,0, 27,16, 5,  5, 3,  'Entry Form',NULL,0,NULL,@UserId,GETDATE() ,@CompanyId)
	...
	MERGE INTO [dbo].[TVA] AS Target 
    USING ( VALUES
        
		(NEWID(), 7.7, @CompanyId, @UserId, GETDATE())
    )
    AS Source (Id, TVAValue, CompanyId, CreatedByUserId, CreatedDateTime)
    ON Target.CompanyId = Source.CompanyId
    WHEN MATCHED THEN 
    UPDATE SET Id = Source.Id,
               TVAValue = Source.TVAValue,
			   CompanyId = Source.CompanyId,
               [CreatedDateTime] = Source.[CreatedDateTime],
               [CreatedByUserId] = Source.[CreatedByUserId]
    WHEN NOT MATCHED BY TARGET THEN 
    INSERT (Id, TVAValue, CompanyId, CreatedByUserId, CreatedDateTime)
    VALUES (Id, TVAValue, CompanyId, CreatedByUserId, CreatedDateTime);

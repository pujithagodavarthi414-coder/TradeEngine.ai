CREATE PROCEDURE [dbo].[Marker455]
(
...
USING ( VALUES 
 (NEWID(),N'By using this app user can upload excel sheet that contain records of the cutom application', N'Custom App Records Excel Uploader', CAST(N'2023-03-17 09:57:46.710' AS DateTime),@UserId,@CompanyId,NULL,NULL,NULL)
 )
...
MERGE INTO [dbo].[WidgetModuleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE WidgetName = 'Custom App Records Excel Uploader' AND CompanyId = @CompanyId),'3DC59A11-F589-42A5-8F7A-925EC9B9542B',@UserId,@Currentdate)
	 )
	...
	MERGE INTO [dbo].[WidgetRoleConfiguration] AS Target 
		USING (VALUES 
	(NEWID(),(SELECT Id FROM Widget WHERE CompanyId = @CompanyId AND WidgetName = 'Custom App Records Excel Uploader'),@RoleId,GETDATE(),@UserId)
	)

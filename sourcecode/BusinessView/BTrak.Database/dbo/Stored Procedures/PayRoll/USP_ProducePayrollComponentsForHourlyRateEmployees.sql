CREATE PROCEDURE [dbo].[USP_ProducePayrollComponentsForHourlyRateEmployees]
(
...
BEGIN
	...
	DECLARE @CompanyId UNIQUEIDENTIFIER = (SELECT [dbo].[Ufn_GetCompanyIdBasedOnUserId](@OperationsPerformedBy)),
	        @Year INT = DATEPART(YEAR,@RunEndDate),
			@ProcessedEmployeesWeightage FLOAT
	...
	SELECT ROW_NUMBER() OVER (ORDER BY E.Id,TS.[Date]) Id,E.Id EmployeeId, E.UserId, TS.[Date], DATENAME(WEEKDAY,TS.[Date]) [DayOfWeek], SW.Id ShiftWeekId, ST.Id ShiftTimimgId, ST.ShiftName, SW.StartTime, SW.EndTime,
	       CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(SW.StartTime AS TIME) AS DATETIME) ShiftStartTime, 
		   CASE WHEN SW.StartTime > SW.EndTime THEN CAST(CAST(DATEADD(DAY,1,TS.[Date]) AS DATE) AS DATETIME) + CAST(CAST(SW.EndTime AS TIME) AS DATETIME) 
		        ELSE CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(SW.EndTime AS TIME) AS DATETIME) END ShiftEndTime,
		   CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(SW.Deadline AS TIME) AS DATETIME) ShiftDeadline, 
		   SW.IsPaidBreak, SW.AllowedBreakTime,
		   CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(TS.StartTime AS TIME) AS DATETIME) InTime, 
		   CASE WHEN CAST(TS.StartTime AS TIME) > CAST(TS.EndTime AS TIME) THEN CAST(CAST(DATEADD(DAY,1,TS.[Date]) AS DATE) AS DATETIME) + CAST(CAST(TS.EndTime AS TIME) AS DATETIME) 
		        ELSE CAST(CAST(TS.[Date] AS DATE) AS DATETIME) + CAST(CAST(TS.EndTime AS TIME) AS DATETIME) END OutTime, 
		   NULL LunchBreakStartTime, NULL LunchBreakEndTime, BreakMins BreakInMin, 
	...
	FROM Employee E
		 ...
		 INNER JOIN TimeSheetPunchCard TS ON TS.UserId = U.Id
		 INNER JOIN [Status] S ON S.Id = TS.StatusId AND S.CompanyId = @CompanyId AND S.StatusName = 'Approved'
		...
		INNER JOIN EmployeeBranch EB ON EB.EmployeeId = E.Id
		      AND ( (EB.ActiveTo IS NOT NULL AND TS.[Date] BETWEEN EB.ActiveFrom AND EB.ActiveTo)
			          OR (EB.ActiveTo IS NULL AND TS.[Date] >= EB.ActiveFrom)
					  OR (EB.ActiveTo IS NOT NULL AND TS.[Date] <= EB.ActiveTo)
				  ) 
		INNER JOIN EmploymentStatus ESS ON ESS.Id = J.EmploymentStatusId AND ESS.InActiveDateTime IS NULL
		INNER JOIN WeekDays WD ON WD.WeekDayName = DATENAME(WEEKDAY,TS.[Date]) AND WD.CompanyId = @CompanyId
		...
		LEFT JOIN ShiftWeek SW ON SW.ShiftTimingId = ST.Id AND SW.[DayOfWeek] = DATENAME(WEEKDAY,TS.[Date])
		LEFT JOIN Holiday H ON H.[Date] = TS.[Date] AND H.CompanyId = @CompanyId AND H.CountryId = B.CountryId
	WHERE ESS.EmploymentStatusName = 'Consultant'
	      AND (@EmployeeIds IS NULL OR E.Id IN (SELECT CAST(Id AS UNIQUEIDENTIFIER) FROM UfnSplit(@EmployeeIds)))
		  AND TS.[Date] BETWEEN @RunStartDate AND @RunEndDate
	ORDER BY ES.EmployeeId,TS.[Date]

	UPDATE #EmployeeDetails SET ShiftStartTime = CAST(CAST(ED.[Date] AS DATE) AS DATETIME) + CAST(CAST(SE.StartTime AS TIME) AS DATETIME), 
		                        ShiftEndTime = CASE WHEN SE.StartTime > SE.EndTime THEN CAST(CAST(DATEADD(DAY,1,ED.[Date]) AS DATE) AS DATETIME) + CAST(CAST(SE.EndTime AS TIME) AS DATETIME) 
		                                            ELSE CAST(CAST(ED.[Date] AS DATE) AS DATETIME) + CAST(CAST(SE.EndTime AS TIME) AS DATETIME) END,
		                        ShiftDeadline = CAST(CAST(ED.[Date] AS DATE) AS DATETIME) + CAST(CAST(SE.Deadline AS TIME) AS DATETIME), 
		                        AllowedBreakTime = SE.AllowedBreakTime, StartTime = SE.StartTime, EndTime = SE.EndTime
	FROM #EmployeeDetails ED 
	     INNER JOIN ShiftException SE ON SE.ExceptionDate = ED.[Date] AND SE.ShiftTimingId = ED.ShiftTimimgId AND SE.ExceptionDate BETWEEN @RunStartDate AND @RunEndDate
		 INNER JOIN (SELECT ShiftTimingId,ExceptionDate,MAX(CreatedDateTime) CreatedDateTime FROM ShiftException WHERE ExceptionDate BETWEEN @RunStartDate AND @RunEndDate 
		             GROUP BY ShiftTimingId,ExceptionDate) SEInner ON SEInner.ShiftTimingId = SE.ShiftTimingId AND SEInner.ExceptionDate = SE.ExceptionDate AND SEInner.CreatedDateTime = SE.CreatedDateTime
	WHERE SE.InActiveDateTime IS NULL
	...
	BEGIN
		...
		SELECT @InTime = DATEADD(MINUTE,@OffSetInMinWithSign,@InTime), @OutTime = DATEADD(MINUTE,@OffSetInMinWithSign,@OutTime),
		       @CameLateAllowanceMaxTime = DATEADD(MINUTE,@OffSetInMinWithSign,@CameLateAllowanceMaxTime)
	...
	DECLARE @EmployeesCounter INT = 1, @EmployeesCount INT, @EmployeeName NVARCHAR(500), @TotalDaysInPayroll INT, @TotalWorkingDays INT, @LeavesTaken INT, @EmployeeBonus FLOAT,
	        @BonusId UNIQUEIDENTIFIER, @BonusName NVARCHAR(50), @PayrollRunMonth INT = DATEPART(MONTH,@RunEndDate), @Salary FLOAT, @IsTdsRequired BIT,
			@TaxFinancialYearFromMonth INT, @TaxFinancialYearToMonth INT, @TaxFinancialYearTypeId UNIQUEIDENTIFIER, @TaxFinancialYearFromDate DATE, @TaxFinancialYearToDate DATE, @TaxId UNIQUEIDENTIFIER, @TaxName NVARCHAR(50), 
			@TaxFinancialFromYear INT, @TaxFinancialToYear INT, @MaxTaxableAmount FLOAT, @TaxablePercentage FLOAT, @TotalPaidAmount FLOAT, @PreviousTaxAmount FLOAT, @TaxAmount FLOAT,
			@LoanId UNIQUEIDENTIFIER, @LoanName NVARCHAR(50), @LoanAmount FLOAT, @SalaryId UNIQUEIDENTIFIER, @SalaryName NVARCHAR(50),
			@EmployeeCountryId UNIQUEIDENTIFIER

	SELECT @EmployeesCount = COUNT(1) FROM #EmployeeSalary

	WHILE(@EmployeesCounter <= @EmployeesCount)
	BEGIN

		SELECT @TaxFinancialYearFromMonth = nULL, @TaxFinancialYearToMonth = NULL

		SELECT @EmployeeId = EmployeeId, @Salary = Salary FROM #EmployeeSalary WHERE Id = @EmployeesCounter

		SELECT @EmployeeName = U.FirstName + ' ' + ISNULL(U.SurName,'') FROM [User] U INNER JOIN Employee E ON E.UserId = U.Id AND E.Id = @EmployeeId

		SELECT @TotalDaysInPayroll = DATEDIFF(DAY,@RunStartDate,@RunEndDate) + 1

		SELECT @TotalWorkingDays = COUNT(DISTINCT [Date]) FROM #EmployeeDetails WHERE EmployeeId = @EmployeeId

		SELECT @LeavesTaken = @TotalDaysInPayroll - @TotalWorkingDays
		...
		INSERT INTO Bonus
		SELECT CASE WHEN Bonus IS NOT NULL THEN Bonus ELSE (CASE WHEN IsCtcType = 1 THEN ROUND(((SELECT TOP 1 Salary FROM #EmployeeSalary WHERE EmployeeId = @EmployeeId) * Percentage * 0.01),0)
		...
		BEGIN
			...
			INSERT INTO #Payroll
			SELECT TOP 1 ES.EmployeeId,@EmployeeName,NULL,@BonusId,@BonusName,0,ES.Salary,ES.Salary,NULL OneDaySalary,0,@EmployeeBonus,@EmployeeBonus,@EmployeeBonus,NULL,@EmployeeBonus,NULL,@TotalDaysInPayroll,
		                 @TotalDaysInPayroll TotalWorkingDays,@LeavesTaken,NULL UnplannedHolidays,NULL SickDays,@TotalWorkingDays WorkedDays,NULL PlannedHolidays,@LeavesTaken LossOfPay,0 IsInResignation,0 TotalLeaves,NULL PaidLeaves,@LeavesTaken UnPaidLeaves,0 AllowedLeaves,
						 IIF(@LeavesTaken > 0,1,0) IsLossOfPayMonth,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
		...
		SELECT @IsTdsRequired = IsTdsRequired FROM TdsSettings 
		WHERE BranchId = @EmployeeBranchId
		...
		SELECT @MaxTaxableAmount = MaxLimit, @TaxablePercentage = [TaxPercentage] FROM [HourlyTdsConfiguration]
		WHERE BranchId = @EmployeeBranchId
		      AND ( (ActiveTo IS NOT NULL AND @RunStartDate BETWEEN ActiveFrom AND ActiveTo AND @RunEndDate BETWEEN ActiveFrom AND ActiveTo)
			          OR (ActiveTo IS NULL AND @RunEndDate >= ActiveFrom)
					  OR (ActiveTo IS NOT NULL AND @RunStartDate <= ActiveTo AND @RunStartDate >= DATEADD(MONTH, DATEDIFF(MONTH, 0, ActiveFrom), 0))
		...
		SELECT @TaxId = Id, @TaxName = ComponentName FROM PayrollComponent WHERE CompanyId = @CompanyId AND ComponentName = 'TDS' AND InActiveDateTime IS NULL
		...
		BEGIN
			...
			SELECT @TaxFinancialYearFromMonth = ISNULL(@TaxFinancialYearFromMonth,4), 
			...
			SELECT @TotalPaidAmount = SUM(PRE.EmployeeSalary)
			FROM PayrollRunEmployee PRE 
			     INNER JOIN PayrollRun PR ON PR.Id = PRE.PayrollRunId
				 INNER JOIN (SELECT PR.PayrollStartDate, PR.PayrollEndDate, MAX(PRE.CreatedDateTime) CreatedDateTime
							 FROM PayrollRunEmployee PRE 
							      INNER JOIN PayrollRun PR ON PR.Id = PRE.PayrollRunId
							 ...
							 WHERE PR.PayrollStartDate BETWEEN @TaxFinancialYearFromDate AND @TaxFinancialYearToDate
							       AND PR.PayrollStartDate < @RunStartdate AND PRE.EmployeeId = @EmployeeId
							 GROUP BY PR.PayrollStartDate, PR.PayrollEndDate) PREInner ON PREInner.PayrollStartDate = PR.PayrollStartDate AND PREInner.PayrollEndDate = PR.PayrollEndDate AND PREInner.CreatedDateTime = PRE.CreatedDateTime AND PRE.EmployeeId = @EmployeeId
					
			SELECT @TotalPaidAmount = ISNULL(@TotalPaidAmount,0) + ISNULL(@Salary,0)
			
			IF(@TotalPaidAmount > @MaxTaxableAmount)
			BEGIN

				SELECT @TaxAmount = (@TotalPaidAmount - @MaxTaxableAmount) * @TaxablePercentage * 0.01
				
				SELECT @PreviousTaxAmount = PEC.ActualComponentAmount
				...
				SELECT @TaxAmount = ROUND((@TaxAmount + ISNULL(@PreviousTaxAmount,0)),0)
			...
			BEGIN
				...
				INSERT INTO #Payroll
				SELECT TOP 1 ES.EmployeeId,@EmployeeName,NULL,@TaxId,@TaxName,1,ES.Salary,ES.Salary,NULL OneDaySalary,0,-@TaxAmount,-@TaxAmount,NULL,-@TaxAmount,NULL,-@TaxAmount,@TotalDaysInPayroll,
				             @TotalDaysInPayroll TotalWorkingDays,@LeavesTaken,NULL UnplannedHolidays,NULL SickDays,@TotalWorkingDays WorkedDays,NULL PlannedHolidays,@LeavesTaken LossOfPay,0 IsInResignation,0 TotalLeaves,NULL PaidLeaves,@LeavesTaken UnPaidLeaves,0 AllowedLeaves,
						 IIF(@LeavesTaken > 0,1,0) IsLossOfPayMonth,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
				FROM #EmployeeSalary ES
			    WHERE ES.EmployeeId = @EmployeeId
		...
		BEGIN
			...
			INSERT INTO #Payroll
			SELECT TOP 1 ES.EmployeeId,@EmployeeName,NULL,@LoanId,@LoanName,1,ES.Salary,ES.Salary,NULL OneDaySalary,0,-@LoanAmount,-@LoanAmount,NULL,-@LoanAmount,NULL,-@LoanAmount,@TotalDaysInPayroll,
			             @TotalDaysInPayroll TotalWorkingDays,@LeavesTaken,NULL UnplannedHolidays,NULL SickDays,@TotalWorkingDays WorkedDays,NULL PlannedHolidays,@LeavesTaken LossOfPay,0 IsInResignation,0 TotalLeaves,NULL PaidLeaves,@LeavesTaken UnPaidLeaves,0 AllowedLeaves,
					 IIF(@LeavesTaken > 0,1,0) IsLossOfPayMonth,NULL,NULL,NULL,NULL,NULL,NULL,NULL,@LoanTotalPaidAmount,@LoanBalanceAmount
			FROM #EmployeeSalary ES
			WHERE ES.EmployeeId = @EmployeeId
		...
		--IF((SELECT COUNT(1) FROM #Payroll WHERE EmployeeId = @EmployeeId) <= 0) 
		--BEGIN

			SELECT @SalaryId = Id, @SalaryName = ComponentName FROM PayrollComponent WHERE CompanyId = @CompanyId AND ComponentName = 'Salary' AND InActiveDateTime IS NULL

			INSERT INTO #Payroll
			SELECT TOP 1 ES.EmployeeId,@EmployeeName,NULL,@SalaryId,@SalaryName,0,ES.Salary,ES.Salary,NULL OneDaySalary,0,ES.Salary,ES.Salary,ES.Salary,NULL,ES.Salary,NULL,@TotalDaysInPayroll,
		                 @TotalDaysInPayroll TotalWorkingDays,@LeavesTaken,NULL UnplannedHolidays,NULL SickDays,@TotalWorkingDays WorkedDays,NULL PlannedHolidays,@LeavesTaken LossOfPay,0 IsInResignation,0 TotalLeaves,NULL PaidLeaves,@LeavesTaken UnPaidLeaves,0 AllowedLeaves,
						 IIF(@LeavesTaken > 0,1,0) IsLossOfPayMonth,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL,NULL
			FROM #EmployeeSalary ES
			WHERE ES.EmployeeId = @EmployeeId
	...
	--SELECT * FROM #EmployeeDetails ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeRateTag ORDER BY EmployeeId,[Date]
	--SELECT * FROM #DaysOfWeekConfiguration ORDER BY EmployeeId,[Date]
	--SELECT * FROM #RateTagDetails ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeRates ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeFinalRates1 ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeFinalRates ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeDateLevelSalary ORDER BY EmployeeId,[Date]
	--SELECT * FROM #EmployeeSalary ORDER BY EmployeeId
	SELECT * FROM #Payroll ORDER BY EmployeeName,IsDeduction

END
GO

--EXEC [USP_ProducePayrollComponentsForHourlyRateEmployees] 'E3D4C1FE-A9B6-4D9B-8FAB-416CFF7D389A',NULL,NULL,'2020-11-02','2020-11-02'

--EXEC [USP_ProducePayrollComponentsForHourlyRateEmployees] 'ED862F40-DCBC-4FAF-824B-4AA2C732123C',NULL,NULL,'2020-08-01','2020-08-31'


--EXEC [USP_ProducePayrollComponentsForHourlyRateEmployees] 'D480B760-362F-464C-A2E2-F068AA9C23DF',NULL,NULL,'2020-11-02','2020-11-02'
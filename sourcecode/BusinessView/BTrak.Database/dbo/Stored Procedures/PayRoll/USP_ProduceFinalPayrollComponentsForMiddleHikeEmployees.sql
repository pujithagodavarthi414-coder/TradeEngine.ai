CREATE PROCEDURE [dbo].[USP_ProduceFinalPayrollComponentsForMiddleHikeEmployees]
(
...
BEGIN
	...
	DECLARE @LossOfPayStartDate DATE, @LossOfPayEndDate DATE,@LOPEmployeesCount INT, @LOPEmployeesCounter INT = 1, @LOPEmployeeId UNIQUEIDENTIFIER,@LOPEmployeeBranchId UNIQUEIDENTIFIER, @LOPUserId UNIQUEIDENTIFIER,  
			...
			@LOPFinancialFromYear INT, @LOPFinancialToYear INT, @LeaveEncashmentPeriodTypeForLop NVARCHAR(100), @LOPEmployeeCountryId UNIQUEIDENTIFIER,@IsEncashCtcType BIT
	...
	CREATE TABLE #EmployeeConfigAndSalaryWithLOP
	(
	...
	BEGIN
		...
		END
			...
			UPDATE #EmployeeConfigAndSalaryWithLOP SET WorkedDays = CASE WHEN IsConfigDates = 1 THEN (CASE WHEN EmployeepayrollConfigurationActiveTo IS NOT NULL THEN DATEDIFF(DAY,@RunStartDate,EmployeepayrollConfigurationActiveTo) + 1 ELSE DATEDIFF(DAY,EmployeepayrollConfigurationActiveFrom,@RunEndDate) + 1 END)
		                                                                                            ELSE (CASE WHEN EmployeeSalaryActiveTo IS NOT NULL THEN DATEDIFF(DAY,@RunStartDate,EmployeeSalaryActiveTo) + 1 ELSE DATEDIFF(DAY,EmployeeSalaryActiveFrom,@RunEndDate) + 1 END) END
			FROM #EmployeeConfigAndSalaryWithLOP ECS 
			WHERE EmployeeId = @LOPEmployeeId

			UPDATE #EmployeeConfigAndSalaryWithLOP SET LOP = CASE WHEN WorkedDays < ED.LossOfPay THEN WorkedDays ELSE ED.LossOfPay END
			FROM #EmployeeConfigAndSalaryWithLOP ECS 
				 ...
				 INNER JOIN #EmployeeDetails ED ON ED.EmployeeId = ECS.EmployeeId
			WHERE ECS.EmployeeId = @LOPEmployeeId

			UPDATE #EmployeeConfigAndSalaryWithLOP SET RestOfLop = CASE WHEN ECSInner.Lop < ED.LossOfPay THEN ED.LossOfPay - ECSInner.LOP ELSE NULL END,
			                                           EncashedLeaves = ED.EncashedLeaves
			FROM #EmployeeConfigAndSalaryWithLOP ECS 
			     INNER JOIN (SELECT EmployeeId, SUM(LOP) LOP FROM #EmployeeConfigAndSalaryWithLOP WHERE EmployeeId = @LOPEmployeeId GROUP BY EmployeeId) ECSInner ON ECSInner.EmployeeId = ECS.EmployeeId
				 INNER JOIN #EmployeeDetails ED ON ED.EmployeeId = ECS.EmployeeId
			WHERE ECS.EmployeeId = @LOPEmployeeId

			UPDATE #EmployeeConfigAndSalaryWithLOP SET LOP = RestOfLop
			FROM #EmployeeConfigAndSalaryWithLOP ECS 
		...
		UPDATE #EmployeeConfigAndSalaryWithLOP SET IsLossOfPayMonth = 1
	...
	SELECT TInner.*,
		   ...
		   UnplannedHolidays,
		   SickDays,
		   CASE WHEN IsLossOfPayMonth = 1 THEN LOP ELSE NULL END LossOfPay,
		   ECSL.EncashedLeaves,
		   PlannedHolidays,
		   AllowedLeaves,
		   TotalLeaves,
	...
	BEGIN
		...
		SELECT TOP 1 @RemainingLeaves = EncashedLeaves FROM #Payroll WHERE EmployeeId = @EmployeeId AND EmployeeConfigAndSalaryId = @EmployeeConfigAndSalaryId 
		...
		WHERE EmployeeConfigAndSalaryId NOT IN (SELECT MAX(EmployeeConfigAndSalaryId) FROM #EmployeeConfigAndSalaryWithLOP WHERE EmployeeId = @EmployeeId)
		      AND EmployeeId = @EmployeeId AND PayrollComponentId = @TaxId -- Duplicate tax is removed from payroll amounts
		...
		--UPDATE #Payroll SET PayrollComponentAmount = 0, ActualPayrollComponentAmount = 0, Deduction = 0, ActualDeduction = 0
		--WHERE EmployeeConfigAndSalaryId NOT IN (SELECT MAX(EmployeeConfigAndSalaryId) FROM #EmployeeConfigAndSalaryWithLOP WHERE EmployeeId = @EmployeeId)
		--      AND EmployeeId = @EmployeeId AND PayrollComponentId = @LeaveEncashmentId -- Duplicate leave encashment is removed from payroll amounts
...
--EXEC USP_ProduceFinalPayrollComponentsForMiddleHikeEmployees '1558A8BC-D112-4FA6-AF52-BD72BBE90F5B',NULL,NULL,'2020-05-01','2020-05-31',NULL,NULL,'[{"EmployeeNumber":"hikes123","EmployeeName":"Peter","LossOfPay":2,"EncashedLeaves":1},{"EmployeeNumber":"branch 123","EmployeeName":"Test","LossOfPay":0,"EncashedLeaves":2}]'
--EXEC USP_ProduceFinalPayrollComponentsForMiddleHikeEmployees '1558A8BC-D112-4FA6-AF52-BD72BBE90F5B',NULL,NULL,'2020-05-01','2020-05-31',NULL,NULL,'[{"EmployeeNumber":"Manually123","EmployeeName":"Peter","LossOfPay":17,"EncashedLeaves":0}]'
--EXEC USP_ProduceFinalPayrollComponentsForMiddleHikeEmployees '1558A8BC-D112-4FA6-AF52-BD72BBE90F5B',NULL,NULL,'2020-05-01','2020-05-31',NULL,NULL,'[{"EmployeeNumber":"Manually123","EmployeeName":"Peter","LossOfPay":1,"EncashedLeaves":3}]'
...
#pragma warning restore 618
        ...
        public void Init(HttpApplication application)
        {
            ...
            EventHandlerTaskAsyncHelper postAuthorizeHelper = new EventHandlerTaskAsyncHelper(PostAuthorizeRequest);
        ...
        protected virtual Task PostAuthorizeRequest(object sender, EventArgs e)
        {
            ...
            return ProcessImageAsync(context);
        }
        ...
        private void PostReleaseRequestState(object sender, EventArgs e)
        {
            ...
            if (imageCache != null)
            {
                SetHeaders(context, imageCache.BrowserMaxDays);
            }
        }
        ...
        private async Task ProcessImageAsync(HttpContext context)
        {
            try
            {
                ...
                string rawUrl = GetRequestUrl(request);

                List<string> ignoreList = new List<string> {"/CONTENT"};
                // Should we ignore this request?
                if (string.IsNullOrWhiteSpace(rawUrl) || rawUrl.ToUpperInvariant().Contains("IPIGNORE=TRUE") || ignoreList.Any(rawUrl.ToUpperInvariant().Contains))
                {
                    return;
                }
                ...
                IImageService currentService = GetImageServiceForRequest(url, applicationPath);
                ...
                if (currentService != null)
                {
                    ...
                    queryString = ReplacePresetsInQueryString(queryString);
                    ...
                    //  LEGACY:
#pragma warning disable 618
                    queryString = CheckQuerystringHandler(context, queryString, rawUrl);
#pragma warning restore 618
                    ...
                    using (await Locker.LockAsync(rawUrl))
                    {
                        ...
                        imageCache = (IImageCache)ImageProcessorConfiguration.Instance
                            .ImageCache.GetInstance(requestPath, url, queryString);

                        // Is the file new or updated?
                        bool isNewOrUpdated = await imageCache.IsNewOrUpdatedAsync();
                        string cachedPath = imageCache.CachedPath;
                        ...
                        if (isNewOrUpdated)
                        {
                            ...
                            catch (HttpException exception)
                            {
                                ...
                                {
                                    ImageProcessorBootstrapper.Instance.Logger.Log<ImageProcessingModule>(exception.Message);
                                    return;
                                }
                            }
                            ...
                            using (MemoryStream inStream = MemoryStreamPool.Shared.GetStream("inStream", imageBuffer, 0, imageBuffer.Length))
                            {
                                ...
                                if (!string.IsNullOrWhiteSpace(queryString))
                                {
                                    ...
                                    AnimationProcessMode mode = ParseAnimationMode(queryString, out processAnimation);
                                    ...
                                    else if (ParseCacheBuster(queryString))
                                    {
                                ...
                                // Add to the cache.
                                await imageCache.AddImageToCacheAsync(outStream, mimeType);
                        ...
                        imageCache.RewritePath(context); // 
                        ...
                        {
                            // Trim the cache.
                            await imageCache.TrimCacheAsync();
                        }
                    }
                }
            }
            ...
            {
                LoggingManager.Error(string.Format(LoggingManagerAppConstants.LoggingManagerErrorValue, "ProcessImageAsync", "ImageProcessingModuleClass", exception.Message), exception);
            }
        }

    ...
    internal sealed class RemoteFile
    {
        ...
        internal async Task<WebResponse> GetWebResponseAsync()
        {
            ...
            catch (WebException exception)
            {
                ...
                if (errorResponse?.StatusCode == HttpStatusCode.NotFound || exception.Status == WebExceptionStatus.NameResolutionFailure)
                {
                    throw new HttpException((int)HttpStatusCode.NotFound, "No image exists at " + this.Uri, exception);
                }
